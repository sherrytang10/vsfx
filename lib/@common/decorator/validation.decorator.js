import { __DefinePrivateProperty, isObject, isBoolean, isString, isFunction } from '../utils';
...
export const IsNotEmpty = (option) => {
    return (target, name, descriptor) => {
        ...
        // }
        // return descriptor;
        descriptor.initializer = () => async(variable) => {
            let result = await __returnCompa(descriptor)
            return Object.assign(result, __returnResult(variable, 'isNotEmpty', { option, _default: `'${name}'不能为空`, name }));
        }
...
export const IsEmail = (option) => {
    return (target, name, descriptor) => {
        // descriptor.initializer = () => variable => __returnResult(variable, 'isEmail', { option, _default: `email格式不正确,xxx@xx.xxx`, name });
        // return descriptor;
...
export const Length = (...arg) => {
    ...
    if (arg.length > 2) {
        option = __getOptions(arg.pop());
    }
    if (arg.length == 2) {
        let [arg1, arg2] = arg;
        if (__Validate.isInterger(arg1) && __Validate.isInterger(arg2)) {
            option = __getOptions({ maxLength: arg2, minLength: arg1 });
        } else if (__Validate.isInterger(arg1)) {
            if (isString(arg2)) {
                arg2 = { message: arg2 };
                option = __getOptions({ length: arg1, message: arg2 });
            } else if (isBoolean(arg2)) {
                option = __getOptions({ length: arg1, required: arg2 });
            } else if (isObject(arg2)) {
                option = __getOptions(arg2);
            } else {
                argErr = false;
                option = `validate @Length argument 2 type is err`;
        ...
        argErr = false;
        option = `validate @Length argument type is err`;
 ...
 * @returns 
 */
function __publicCheck(variable, option, name) {
    ...
    if (__Validate.isNotEmpty(length)) {
        // 存在长度
        if (__Validate.isInterger(length)) {
        ...
        // 不存在长度 但是存在最大和最小长度
        if (__Validate.isInterger(maxLength)) {
            return __errmsg(`maxLength 必须是int类型`);
        }
        if (__Validate.isInterger(minLength)) {
 ...
 * @param {any} cb 
 */
function __returnCompa(descriptor) {
...
function __returnResult(variable, valiFn, params) {
    ...
    if (isObject(option)) {
        // 只会返回true 或者err对象
        let checkVal = __publicCheck(variable, option, name);
        if (checkVal) {
            return checkVal;
        }
 ...
 * @returns 
 */
function __errmsg(message = '验证体入参格式错误') {
