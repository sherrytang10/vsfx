...
export async function doFormidable(req, options = 'images') {
    let rResult = {};
    try {
        if (typeof options == 'string') {
            options = { type: options }
        }
        let result = await _formidableInit(req, {
            uploadDir: path.join(uploadConfig.uploadDir, options.type)
        });
        if (!result.fields) {
            throw new Error('ErrCode: 3,' + result.err, '200');
        }
        let file = result.files.file;
        if (file.size == 0) {
            throw new Error('ErrCode: 3,' + "上传文件不能为空~", '200');
        }
        ...
        try {
            if (options.reg && !options.reg.test(filePath.ext)) {
                ...
                throw new Error('ErrCode: 3,' + "文件格式不合法~", '200');
            }
        } catch (e) {
            throw new Error('ErrCode: 3,' + "文件类型校验失败~", '200');
        }
        return {
        ...
        throw new Error('ErrCode: 3,' + e.message, '200');
    }
}